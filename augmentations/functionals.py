...
def rotate_coords(coords, angle_x, angle_y, angle_z):
    ...
    t = torch.from_numpy(coords.reshape(len(coords), -1).transpose().astype(np.float32))
    rot_matrix = torch.from_numpy(rot_matrix.astype(np.float32))
    coords = torch.matmul(t,rot_matrix).numpy().transpose().reshape(coords.shape)
